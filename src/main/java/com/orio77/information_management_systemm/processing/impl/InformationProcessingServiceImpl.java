...
public class InformationProcessingServiceImpl implements InformationProcessingService {
    ...
    private final static String PROCESS_INFO_USER_MESSAGE = """
            ...
            *   **The Lesson:** Success is non-linear. Once you start winning, the "Matthew Principle" takes over: "To those who have everything, more will be given." Small advantages at the beginning of a cycle (like a slightly better posture or a small win) compound into massive differences over time. Conversely, failures also compound. </concept>
            ...
            Concept: {{INSERT_CORE_IDEA_HERE}}
            Original_Text: {{INSERT_ORIGINAL_CHAPTER_TEXT_HERE}}
                        """;

    @Override
    public List<List<Generation>> processInformation(List<String> information, String sourceContent) {

        // Prepare a list to hold processed information
        List<List<Generation>> res = new ArrayList<>();

        // Process each PDDocument in the input list
        for (String info : information) {
            // Build and send the prompt to the chat model
            ChatResponse response = chatModel.call(buildProcessingPrompt(info, sourceContent));
            // Retrieve and store the result
            res.add(response.getResults());

            // TODO remove
            break;
        }

        log.info("Processed information count: {}", res.size());

        return res;
    }

    private Prompt buildProcessingPrompt(String coreIdea, String originalText) {
        // Construct the user message by inserting the text
        String userMessage = PROCESS_INFO_USER_MESSAGE.replace("{{INSERT_CORE_IDEA_HERE}}", coreIdea)
                .replace("{{INSERT_ORIGINAL_CHAPTER_TEXT_HERE}}", originalText);
